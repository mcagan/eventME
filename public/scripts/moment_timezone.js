!(function (c, M) {
  ...
  function o(c) {
    var M = 0,
      ...
      p = 0,
      z = 1;
    for (45 === c.charCodeAt(0) && (z = -(M = 1)); M < A.length; M++)
  ...
  function L(c, M) {
    for (var a, A; (A = 6e4 * (((M.at - c.at) / 12e4) | 0)); )
      (a = new t(new Date(c.at + A))).offset === c.offset ? (c = a) : (M = a);
    return c;
  }
  function u(c, M) {
    ...
    var a,
      ...
      b = (function () {
        var c,
          ...
          b = [o];
        for (a = 1; a < 48; a++)
          (M = new t(new Date(A, a, 1))).offset !== o.offset &&
            ((c = L(o, M)), b.push(c), b.push(new t(new Date(c.at + 6e4)))),
            (o = M);
        for (a = 0; a < 4; a++)
          b.push(new t(new Date(A + a, 0, 1))),
            b.push(new t(new Date(A + a, 6, 1)));
        return b;
      })(),
      p = b.length,
      z = (function (c) {
        ...
        for (M = 0; M < o; M++)
          for (a in (A = e[c[M].offset] || {}))
            A.hasOwnProperty(a) && (b[a] = !0);
        for (M in b) b.hasOwnProperty(M) && p.push(n[M]);
        return p;
      })(b),
      i = [];
    for (A = 0; A < z.length; A++) {
      for (a = new l(m(z[A]), p), o = 0; o < p; o++) a.scoreOffsetAt(b[o]);
      i.push(a);
    }
    return i.sort(u), 0 < i.length ? i[0].zone.name : void 0;
  }
  function X(c) {
  ...
  function m(c, M) {
    ...
    return A instanceof f
      ...
      ? ((A = new f(A)), (p[c] = A))
      : z[c] && M !== m && (a = m(z[c], m))
      ? ((A = p[c] = new f())._set(a), (A.name = n[c]), A)
      : null;
  }
  function T(c) {
    var M, a, A, o;
  ...
  (a < 2 || (2 == a && A < 6)) &&
    ...
    (C.names = function () {
      var c,
        M = [];
      for (c in n)
        n.hasOwnProperty(c) && (p[c] || p[z[c]]) && n[c] && M.push(n[c]);
      return M.sort();
    }),
    (C.Zone = f),
    (C.unpack = d),
    ...
    (C.zonesForCountry = function (c, M) {
      if (
        !(c = (function (c) {
          return (c = c.toUpperCase()), i[c] || null;
        })(c))
      )
        return null;
      var a = c.zones.sort();
      ...
      if (
        (void 0 === c._z &&
          (A &&
            R(c) &&
            !c._isUTC &&
            ((c._d = b.utc(c._a)._d), c.utc().add(A.parse(c), "minutes")),
          (c._z = A)),
        c._z)
      )
        if (
